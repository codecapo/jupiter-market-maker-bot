...
export class AppService {
  ...
  public async deleteCurrentInsertNewRandomSwapAmount(
    randomSwapAmounts: NewRandomSwapAmountRequestDto[],
  ): Promise<any> {
    const mapRequests = randomSwapAmounts.map((item) => {
      const mappedItem: RandomSwapAmount = {
        incrementPositionKey: item.postion,
        swapAmountValue: item.amount,
      };
      return mappedItem;
    });

    return await this.appRepo.deleteExistingAndInsertNewRandomSwapAmounts(
      mapRequests,
    );
  }
  ...
  public async mapSwapRequests() {
    const marketMakerWallets =
      await this.appRepo.findStartOldestUnprocessedOrder();
      ...
      const serviceRequests = mappedExecutionOrderToSwapRequests.map(
        (request) => {
          try {
            ...
            this.logger.debug(
              `Executing swap for public key: ${keypair.publicKey.toBase58()}`,
            );
